...
#      done
  ...
  echo ""
  echo "This Workshop require at least one user with admin role and, if multiuser mode is selected (default) a dynamic PV that is used by the spawner of the workshop environments. You can deploy automatically the pre-requirements using --prerequisites, it creates local user accounts and deploy the (unsupported) dynamic NFS autoprovisioner (that must be configured pointing to the NFS server IP before running the script)".
  echo "If no dynamic PV is possible in this environment, use the --singleuser option but be aware that in that case the Web Console will use a Service Account instead of the user account, and that could affect some of the steps described in the Workshop"
  echo ""
...
###
# NOTE: I'm using an image with the workshop already built. If you want to make any changes you need to run .workshop/scripts/build-workshop.sh after making the changes. You can also start from the "base" image instead from the one hosted in my quay account, you need to change the WORKSHOP_IMAGE variable in the .workshop/settings.sh script to use, for example, quay.io/openshifthomeroom/lab-markdown-sample:1.10
###
#sleep 30
